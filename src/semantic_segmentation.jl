...
- `is_autolabel` (optional, default false): Whether to use the autolabeling parameter file instead of the segmentation parameter file.
"""
function call_unet(param_path::Dict; gpu=0, is_autolabel=false)
    if is_autolabel
        path_root_process = param_path["path_root_process_label"]
    else
    ...
    path_sh = joinpath(path_root_process, "run_$(unet_str).sh")
    path_log = joinpath(path_root_process, "$(unet_str).log")

    if is_autolabel
        path_unet_data = param_path["path_dir_autolabel_input"]
    else
    ...
    if is_autolabel
        path_unet_param = param_path["path_autolabel_param"]
    else
